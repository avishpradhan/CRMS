...
import {
  Eye, Edit3, Trash2, MapPin, GraduationCap, Calendar,
  Users, Code, IndianRupee, FileText, Save, Send,
  Paperclip, File, Image,
} from 'lucide-react';
...
/* ────────── Edit Modal ────────── */
function DriveEditModal({ drive, isOpen, onClose, onSave }) {
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Edit Drive" size="lg">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex gap-3 pt-2">
          <button type="submit" className="btn-primary flex-1"><Save size={16} /> Save Changes</button>
...
export default function AdminDrives() {
  ...
  const [floodConfirmDrive, setFloodConfirmDrive] = useState(null);
  ...
  const fetchDrives = async () => {
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        throw new Error(data.message || 'Failed to fetch campus drives');
      }
  ...
  const handleFloodDrive = (drive) => {
    setFloodConfirmDrive(drive);
  };

  const handleConfirmFlood = async () => {
    if (!floodConfirmDrive) return;
    try {
      setFloodLoading(true);
      const res = await fetch(`/api/admin/drives/${floodConfirmDrive.id}/flood`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      ...
      const data = await res.json();
      if (!res.ok) {
        throw new Error(data.message || 'Failed to flood drive');
      }

      setFloodConfirmDrive(null);
      try {
        alert(data.message || 'Drive flooded successfully!');
      } catch (ae) {
        console.warn('alert() is blocked in this environment:', ae);
      }
    } catch (err) {
      try {
        alert(err.message);
  ...
  const columns = [
    { header: 'Company', accessor: 'companyName', render: (row) => (
      <div>
        ...
        <p className="text-xs text-primary-500">{row.role}</p>
      </div>
    )},
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-surface-900 dark:text-white">Campus Drives</h1>
          <p className="text-surface-500 dark:text-surface-400 mt-1">{drivesList.length} drives total</p>
        </div>
      </div>
      ...
      {/* Flood Confirmation Modal */}
      <Modal
        isOpen={!!floodConfirmDrive}
        onClose={() => setFloodConfirmDrive(null)}
        title="Confirm Notification"
        size="sm"
      >
        <div className="space-y-4">
          <p className="text-sm text-surface-600 dark:text-surface-400">
            Are you sure you want to notify all eligible students about <strong className="text-surface-900 dark:text-white">{floodConfirmDrive?.role}</strong> at <strong className="text-surface-900 dark:text-white">{floodConfirmDrive?.companyName}</strong> in one-click?
          </p>
          <div className="p-3 bg-primary-50 dark:bg-primary-500/10 text-primary-600 dark:text-primary-400 text-xs rounded-xl border border-primary-200 dark:border-primary-800">
            <strong>Info:</strong> This will send automated email/dashboard notifications to all students matching the criteria for this drive.
          </div>
          <div className="flex gap-3">
            <button
              onClick={handleConfirmFlood}
              disabled={floodLoading}
              className="btn-primary flex-1"
            >
              {floodLoading ? 'Sending...' : 'Notify Students'}
            </button>
            <button
              onClick={() => setFloodConfirmDrive(null)}
              className="btn-secondary flex-1"
            >
              Cancel
            </button>
